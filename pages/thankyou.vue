<template>
<div class="container px-[20px] mx-auto h-screen flex flex-col justify-center" >
    <div class="flex flex-col items-center">
        <div>
            <img src="/img/done_icon.png" alt="icon" />
        </div>

        <div class="mt-[24px] font-bold text-secondary">
            <h1 class="text-[30px] md:text-[50px]">GRAZIE</h1>
        </div>

        <div class=" mt-[54px] font-bold text-secondary">
            <h2 class="text-center text-[20px] md:text-[36px]">La tua prenotazione è andata a buon fine</h2>
        </div>
    </div>

        <div class="mt-[54px]">
            <hr class=" border-[1px] border-[#B4B4B4] bg-[#B4B4B4] md:mx-[50px]">
        </div>

         <div class="w-full flex justify-center mt-[83px]">
            <button class=" bg-primary rounded-[11px] text-secondary py-[14px] px-20 font-semibold text-[18px] md:text-[24px]"><h3>TORNA IN HOME PAGE</h3></button>
        </div>
</div>
</template>